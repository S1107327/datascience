import pandas as pd
legit = []
with open("legit/5000.txt", "r") as fin:
    line = fin.readline().strip('\n')
    while line:
        legit.append(line)
        line = fin.readline().strip('\n')

labels = ['legit' for i in range(5000)]

legit_df = pd.DataFrame({'label': pd.Series(labels), 'domain': pd.Series(legit)})

# dga part
dga = []
labels_dga = ['dga' for i in range(5000)]
...
dga_df = pd.DataFrame({'label': pd.Series(labels_dga), 'domain': pd.Series(dga)})
